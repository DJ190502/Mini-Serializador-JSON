# Miniserializador JSON en NetBeans

## 📌 Descripción
Este proyecto está desarrollado en **Java** utilizando el entorno de desarrollo integrado (IDE) **NetBeans**.  
El objetivo de este README es guiar al usuario en la instalación, configuración y ejecución del proyecto dentro de NetBeans.

---

## ⚙️ Requisitos previos
- **Java JDK 17+**.
- **NetBeans IDE 12+** (compatible con la versión de JDK).

---

## 🚀 Pasos para ejecutar el proyecto en NetBeans

### 1. Instalar JDK
- Descarga e instala el **Java Development Kit (JDK)** desde la página oficial de Oracle o adoptium.net.
- Verifica la instalación con:
  ```bash
  java -version


### 2. Instalar NetBeans

- Descarga NetBeans desde https://netbeans.apache.org.
- Durante la instalación, asegúrate de que NetBeans detecte el JDK instalado.

### 3. Importar el proyecto
- Abre NetBeans.
- Ve a File → Open Project.
- Selecciona la carpeta del proyecto (NombreDelProyecto).
- NetBeans reconocerá automáticamente la estructura del proyecto.

### 4. Compilar y ejecutar
- Haz clic derecho sobre el proyecto → Clean and Build.
- Luego selecciona Run Project (o presiona F6).
- La salida aparecerá en la consola de NetBeans.
